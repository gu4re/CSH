...
bool one_cmd_bash(tline *line, const char * cmd){
	...
	/* Variables locales del subprograma call_bash */
	...
	/* Proceso Hijo */
		...
		/* Activación del manejador Ctrl+C manejando su posible fallo */
		if (line->background){
 ...
 * Aquella que nos permite ejecutar un comando cuando se  *
 * introduce exactamente un unico comando ya sea nuestro  *
 *  		o del propio bash 			  */
bool one_cmd(tline *line, const char * cmd){
	...
	if(strcmp(line->commands[0].argv[0], "cd") == 0){
		...
		/* Es necesario liberar la memoria usada antes de salir de la minishell */
		free_mem();
...
/* 		      doline function 			  *
 *    Aquella que nos permite ejecutar una linea o tline  *
 *         distinguiendo entre uno o mas comandos  	  */
void doline(tline * line, const char * cmd){

	/* C inicializa por defecto las variables de tipo 'int' a 0 */
	int i;
	...
	/* Ajuste de cuando nos meten un intro-vacio o lo que es lo mismo, que la linea sea NULL */
	if (line->ncommands == 0)
		return;
	...
	if (line->ncommands > 1){
		...
		// ls -l | grep p | tr "w" "a" output = stdout input = p0 i 2 
...
/* 		     Handler_MiniShell 		  	  *
 *    Aquel manejador que trata la señal del Ctrl+C	  */
void handler_msh(void){	
...
int main(void){
	...
	/* Variables locales de la minishell */
	char buf[1024];
	tline *line;
	...
	/* Bucle while que lee una linea del STDIN */
        while (fgets(buf, 1024, stdin) != NULL) {
	
		/* Tokenizamos una linea que hemos leido con el fgets */		
                line = tokenize(buf);
